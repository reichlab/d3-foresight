...
The library requires [[https://d3js.org/][d3]] as external dependency. To build foresight itself, use
~npm compile~ (for ~./dist/d3-foresight.js~) or ~npm build~ (for
...
you should be able to move around by clicking the arrow buttons in legend.
Events like mouse clicks and movement arrows can call user defined functions
too. See the section on [[Hooks]] for more description.
