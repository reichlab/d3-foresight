...
/onset/outbreak/ point (as defined by a baseline). This visualizations in this
...
#+HTML: <br><br>
#+HTML: <div><div id="timechart"></div></div>
...
might also be noticing that nothing happens when you /click/ on the chart. This is
because we need to register hook functions for events like clicks. To add the
/on click interactivity/, we need to register a function which calls
~timeChart.update~ on a mouse click event. Press execute button below to set up
the required hook:
...
#+BEGIN_SRC js :exports none :tangle ./assets/script.js
...
section on [[Hooks]] for more description.
